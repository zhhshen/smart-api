...
// 撤销操作
const resetApi = async (bool, LogFileName) => {
  await replaceApi(bool, LogFileName)
}
let configName = 'sapiConf'
let cacheConfigName = 'sapiConf'
let targetFile = `./${configName}.js`
let LogFileName = `./${configName}.log`
const run = {
  init: async function () {
    await run.clean()
    cp(pwd('template/index.js'), `./sapiConf.js`)
  ...
  reset: async function () {
    if (exists(targetFile)) {
      const sapiConf = require(cwd(targetFile))
      await createOpts(sapiConf)
      await resetApi(false, LogFileName)
  ...
  clean: async function () {
    ...
    await deleteFile(cwd(`./${cacheConfigName}.js`)) // 删除sapiConf.js文件
    await deleteFile(cwd(`./${cacheConfigName}.log`)) // 删除sapiConf.log文件
  }
}
module.exports = run